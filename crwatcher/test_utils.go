...
func (m *StatusMatcher) Matches(x interface{}) bool {
	...
	if m.status.Phase != s.Phase {
		m.mismatchedField = "Phase"
	} else if m.status.Reason != s.Reason {
		m.mismatchedField = "Reason"
	} else if m.status.Message != s.Message {
		m.mismatchedField = "Message"
	}
	return m.status.Phase == s.Phase && m.status.Reason == s.Reason && m.status.Message == s.Message
}

func (m StatusMatcher) String() string {
	return "fields \"" + m.mismatchedField + "\" do not match"
}
